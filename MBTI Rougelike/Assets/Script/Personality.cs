...
public class Personality : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        SkillUpdate(normalAttack, ref normalAttack_CurretReloadingTimer, normalAttack_InitPosition, Input.GetMouseButton(0)); //左键
        SkillUpdate(specialSkill, ref specialSkill_CurretReloadingTimer, specialSkill_InitPosition, Input.GetMouseButton(1)); //右键

        UltimateSkillUpdate();
    }

    protected virtual void SkillUpdate(Skill skill, ref float currentReloadingTimer, Transform initPos, bool holding)
    {
        ...
        if (holding) // 在后面改为绑定按键
        {
            ...
            if (currentReloadingTimer <= 0.0f)
            {
                ...
                damageCollider.owner = playerTransform.GetComponent<Unit>();
